...
var moho_db = mongo.db("192.168.1.42/pag", {safe : true});

var MongoApiServer = function(app){

    //assume localhost
    var connect = function(dbName,next){
        var db = mongo.db("192.168.1.42/" + dbName, {safe : true});
        next(db);
    };
    ...
    app.post("/mongo-api/query", function(req,res){
        ...
        moho_db.collection(collection).find(query).toArray(function(err,response){
            res.json(response);
        });
...
//    });
    ...
    app.delete("/mongo-api/moho/:collection/:id",function(req,res){
        console.log('delete mohoitem');
        console.log(req.params.collection);
        console.log(req.params.id);
        moho_db.collection(req.params.collection).removeById(req.params.id,function(err,result){
            console.log(err);
            res.json(result);
        });
    });
